...
class RxBus_Demo_macOS_Tests: XCTestCase {
    ...
    func testSystemNotificationSubscriptionAndPosting() {
        ...
        bus.asObservable(notificationName: NSPasteboard.changedNotification).subscribe { event in
            executeExpectation.fulfill()
        }.disposed(by: disposeBag)
        bus.post(notification: Notification(name: NSPasteboard.changedNotification))
    ...
    func testThreadSafe() {
        ...
        for i in stride(from: 0, to: 100, by: 2) {
            ...
            DispatchQueue(label: "UIPasteboard.changedNotification \(i)").async {
                self.bus.asObservable(notificationName: NSPasteboard.changedNotification)
                    .subscribeOn(MainScheduler.instance)
                    .subscribe { _ in
                        callCount += 1
                    }
            ...
            DispatchQueue(label: "UIPasteboard.changedNotification \(i + 1)").async {
                self.bus.asObservable(notificationName: NSPasteboard.changedNotification, priority: i + 1)
                    .subscribeOn(MainScheduler.instance)
                    .subscribe { _ in
                        callCount += 1
                    }
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 4.0) {
            ...
            self.bus.post(notification: Notification(name: NSPasteboard.changedNotification))
            executeExpectation.fulfill()
        }
